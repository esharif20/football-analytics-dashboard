...
export function useDemoStats(statistics: any, analyticsData: any, demoEvents: any[]) {
  return useMemo(() => {
    if (statistics) {
      return {
        possessionTeam1: toNumber(statistics.possessionTeam1, 0),
        possessionTeam2: toNumber(statistics.possessionTeam2, 0),
        passesTeam1: toNumber(statistics.passesTeam1, 0),
        passesTeam2: toNumber(statistics.passesTeam2, 0),
        shotsTeam1: toNumber(statistics.shotsTeam1, 0),
        shotsTeam2: toNumber(statistics.shotsTeam2, 0),
        distanceCoveredTeam1: toNumber(statistics.distanceCoveredTeam1, 0),
        distanceCoveredTeam2: toNumber(statistics.distanceCoveredTeam2, 0),
        avgSpeedTeam1: toNumber(statistics.avgSpeedTeam1, 0),
        avgSpeedTeam2: toNumber(statistics.avgSpeedTeam2, 0),
        maxSpeedTeam1: toNumber(statistics.maxSpeedTeam1, 0),
        maxSpeedTeam2: toNumber(statistics.maxSpeedTeam2, 0),
        possessionChanges: toNumber(statistics.possessionChanges, 0),
        ballDistance: toNumber(statistics.ballDistance, 0),
        ballAvgSpeed: toNumber(statistics.ballAvgSpeed, 0),
        ballMaxSpeed: toNumber(statistics.ballMaxSpeed, 0),
    ...
    let team1DistanceM = 0,
      team2DistanceM = 0,
      team1TrackedTimeS = 0,
      team2TrackedTimeS = 0
    let team1MaxSpeedMps = 0,
      team2MaxSpeedMps = 0
    for (const pdata of Object.values(playerKinematics) as any[]) {
      ...
      const d = toNumber(pdata.total_distance_m, 0),
        avg = toNumber(pdata.avg_speed_m_per_sec, 0),
        mx = toNumber(pdata.max_speed_m_per_sec, 0)
      if (rawTeamId === 0) {
        team1DistanceM += d
        if (d > 0 && avg > 0) team1TrackedTimeS += d / avg
        team1MaxSpeedMps = Math.max(team1MaxSpeedMps, mx)
      } else {
        team2DistanceM += d
        if (d > 0 && avg > 0) team2TrackedTimeS += d / avg
        team2MaxSpeedMps = Math.max(team2MaxSpeedMps, mx)
      }
    }
    ...
    return {
      possessionTeam1: possTeam1,
      possessionTeam2: possTeam2,
      passesTeam1,
      passesTeam2,
      shotsTeam1,
      shotsTeam2,
      distanceCoveredTeam1: +(team1DistanceM / 1000).toFixed(2),
      distanceCoveredTeam2: +(team2DistanceM / 1000).toFixed(2),
      avgSpeedTeam1:
        team1TrackedTimeS > 0 ? +((team1DistanceM / team1TrackedTimeS) * 3.6).toFixed(1) : 0,
      avgSpeedTeam2:
        team2TrackedTimeS > 0 ? +((team2DistanceM / team2TrackedTimeS) * 3.6).toFixed(1) : 0,
      maxSpeedTeam1: +(team1MaxSpeedMps * 3.6).toFixed(1),
      maxSpeedTeam2: +(team2MaxSpeedMps * 3.6).toFixed(1),
      possessionChanges: toNumber(poss.possession_changes, 0),
      ballDistance: +(ballDistanceM / 1000).toFixed(2),
...
export function useDemoPlayerStats(analyticsData: any, demoEvents: any[]) {
  return useMemo(() => {
    ...
    for (const ev of demoEvents) {
      ...
      if (ev.success !== false)
        passSuccessByPlayer.set(pid, (passSuccessByPlayer.get(pid) ?? 0) + 1)
    }
    const rows = Object.entries(playerKinematics)
      .map(([trackIdRaw, pdata]: [string, any]) => {
        ...
        const distanceM = Number(pdata.total_distance_m),
          avgSpeedMps = Number(pdata.avg_speed_m_per_sec),
          maxSpeedMps = Number(pdata.max_speed_m_per_sec)
        const passes = passAttemptsByPlayer.get(trackId) ?? 0,
          passSuccess = passSuccessByPlayer.get(trackId) ?? 0
        return {
          trackId,
          teamId,
          ...
          passes,
          passAcc: passes > 0 ? Math.round((passSuccess / passes) * 100) : null,
