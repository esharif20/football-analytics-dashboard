...
const ML_EVENT_TYPES = new Set(['challenge', 'play', 'throwin'])
      ...
      <ScrollArea className="h-48">
        <div className="space-y-2">
          {events.slice(0, 10).map((event, i) => {
            const eventConfig = EVENT_TYPES[event.type as keyof typeof EVENT_TYPES]
            const isML = ML_EVENT_TYPES.has(event.type) && event.teamId == null
            return (
              <div
                key={i}
                className="flex items-center gap-3 p-3 rounded-xl bg-secondary/20 border border-border/10 hover:border-border/30 hover:bg-secondary/30 transition-all duration-300"
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-muted-foreground text-xs ml-2">
                    {event.teamId != null ? `Team ${event.teamId} · ` : ''}
                ...
                {isML && event.confidence != null ? (
                  <Badge
                    variant="outline"
                    className="text-xs shrink-0 border-violet-500/30 text-violet-400 bg-violet-500/5"
                  >
                    ML {Math.round(event.confidence * 100)}%
                  </Badge>
                ) : (
                  <Badge
                    variant="outline"
