...
// ==================== Color Constants ====================
const TEAM1_COLOR = "oklch(0.6 0.2 25)";
const TEAM2_COLOR = "oklch(0.55 0.18 200)";
const TEAM1_HEX = "#e05252";
const TEAM2_HEX = "#4a9ede";
const ACCENT_HEX = "#5bc4a8";
const PRIMARY_HEX = "#5bc4a8";
      ...
      <main className="container py-8 relative z-10">
        ...
        {analysis.status === "completed" && (
          <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3 text-xs">
                        <div className="flex items-center gap-1.5">
                          <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: TEAM1_HEX }} />
                          <span className="text-muted-foreground">Team 1</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: TEAM2_HEX }} />
            ...
            {/* Row 2: Charts Row — Possession Donut + Team Performance Radar + Stats Comparison Bar */}
            <div className="grid lg:grid-cols-3 gap-6">
              {/* Possession Donut */}
              <PossessionDonut team1={demoStats.possessionTeam1} team2={demoStats.possessionTeam2} />

              {/* Team Performance Spider/Radar Chart */}
            ...
            {/* Row 3: Team Shape + Defensive Line + Pressing Intensity */}
            <div className="grid lg:grid-cols-3 gap-6">
              <TeamShapeChart />
              <DefensiveLineChart />
            ...
            <div className="grid lg:grid-cols-5 gap-6">
              ...
              <div className="lg:col-span-3">
                <div className="glass-card overflow-hidden">
                  ...
                  <CardContent className="p-4">
                    <ModeSpecificTabs mode={mode} activeTab={activeTab} setActiveTab={setActiveTab} trackingData={demoTrackingData} events={demoEvents} />
                  </CardContent>
                </div>
              </div>
...
// Team Shape / Compactness Over Time (placeholder)
...
// Defensive Line Height Over Time (placeholder)
...
// Pressing Intensity Chart (placeholder)
  ...
  return (
    <div className="pitch-container">
      ...
      {data.team1Players.map((player: any) => (
        <div key={player.id} className="player-marker team-1" style={{ left: `${(player.x / PITCH_WIDTH) * 100}%`, top: `${(player.y / PITCH_HEIGHT) * 100}%` }}>
          <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-white drop-shadow-sm" style={{ zIndex: 1 }}>{player.id}</span>
        </div>
      ))}
      {data.team2Players.map((player: any) => (
        <div key={player.id} className="player-marker team-2" style={{ left: `${(player.x / PITCH_WIDTH) * 100}%`, top: `${(player.y / PITCH_HEIGHT) * 100}%` }}>
          <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-white drop-shadow-sm" style={{ zIndex: 1 }}>{player.id - 11}</span>
        </div>
      ))}
    ...
    <div className="pitch-container">
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 105 68" preserveAspectRatio="xMidYMid meet" style={{ zIndex: 2 }}>
        <defs>
          ...
          <radialGradient id="heatVignette" cx="50%" cy="50%" r="60%"><stop offset="0%" stopColor="transparent" /><stop offset="100%" stopColor="rgba(0,0,0,0.3)" /></radialGradient>
    ...
    <div className="pitch-container">
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 105 68" preserveAspectRatio="xMidYMid meet" style={{ zIndex: 2 }}>
        <defs>
          ...
          <radialGradient id="passVignette" cx="50%" cy="50%" r="60%"><stop offset="0%" stopColor="transparent" /><stop offset="100%" stopColor="rgba(0,0,0,0.3)" /></radialGradient>
        ...
        <g filter="url(#edgeGlow)">
          {edges.map((edge, i) => {
            ...
            return (
              <line key={i} x1={fromNode.x} y1={fromNode.y} x2={toNode.x} y2={toNode.y} stroke="oklch(0.7 0.18 145)" strokeWidth={Math.max(0.5, edge.count / 6)} strokeOpacity={0.5} markerEnd="url(#arrowhead)" />
            );
          })}
        </g>
      ...
      for (let x = 0; x < grid.length; x++) {
        for (let y = 0; y < grid[x].length; y++) {
          if (grid[x][y].playerId === player.id) {
            const isBoundary =
              x === 0 || x === grid.length - 1 || y === 0 || y === grid[x].length - 1 ||
    ...
    <div className="pitch-container">
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 105 68" preserveAspectRatio="xMidYMid meet" style={{ zIndex: 2 }}>
        <defs>
          ...
          <radialGradient id="voronoiVignette" cx="50%" cy="50%" r="60%"><stop offset="0%" stopColor="transparent" /><stop offset="100%" stopColor="rgba(0,0,0,0.3)" /></radialGradient>
        </defs>
    ...
    <div className="space-y-4">
      <div className="relative h-12 bg-secondary/30 rounded-xl overflow-hidden border border-border/20">
        {events.map((event, i) => {
          ...
          return (
            <div key={i} className="event-marker" style={{ left: `${Math.min(position, 98)}%`, backgroundColor: eventConfig?.color || "#666" }} title={`${event.type} at ${event.timestamp}s`} />
          );
        })}
      </div>
      <ScrollArea className="h-48">
        <div className="space-y-2">
          {events.slice(0, 10).map((event, i) => {
            const eventConfig = EVENT_TYPES[event.type as keyof typeof EVENT_TYPES];
            return (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-secondary/20 border border-border/10 hover:border-border/30 hover:bg-secondary/30 transition-all">
...
function ModeSpecificTabs({ mode, activeTab, setActiveTab, trackingData, events }: { mode: PipelineMode; activeTab: string; setActiveTab: (t: string) => void; trackingData: any; events: any[] }) {
  ...
  return (
    <Tabs value={effectiveTab} onValueChange={setActiveTab}>
      ...
      <TabsContent value="radar" className="mt-4"><PitchRadar data={trackingData} /></TabsContent>
