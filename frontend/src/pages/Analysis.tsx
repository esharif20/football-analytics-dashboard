...
interface InteractionNode { playerId: number; teamId: number; avgX: number; avgY: number; avgSpeed: number; passCount?: number; isBall?: boolean; betweenness?: number; clustering?: number; degreeCentrality?: number }
interface InteractionEdge { from: number; to: number; weight: number; passCount?: number; isBallEdge?: boolean }
interface TimelineSegment { label: string; startFrame: number; endFrame: number; edges: InteractionEdge[] }
interface InteractionGraphData { nodes: InteractionNode[]; edges: InteractionEdge[]; timeline?: TimelineSegment[] }
